...
func TestQuery(t *testing.T) {
	t.Run("query with parameters", func(t *testing.T) {
		rows, err := testDB.Query("SELECT * FROM key_values WHERE id BETWEEN $min AND $max", map[string]any{
			"min": 1,
			"max": 2,
		})
		require.NoError(t, err)
	...
	t.Run("query with missing parameter", func(t *testing.T) {
		_, err := testDB.Query("SELECT * FROM key_values WHERE id = $missing", map[string]any{
			"other": 1,
		})
		require.Error(t, err)
	...
	t.Run("query with escaped dollar sign", func(t *testing.T) {
		rows, err := testDB.Query("SELECT '$$test' as literal_dollar", map[string]any{})
		require.NoError(t, err)
		defer rows.Close()
...
func TestExec(t *testing.T) {
	t.Run("exec insert with named parameters", func(t *testing.T) {
		result, err := testDB.Exec("INSERT INTO key_values (`key`, value) VALUES ($key, $value)", map[string]any{
	...
	t.Run("exec delete with named parameters", func(t *testing.T) {
		result, err := testDB.Exec("DELETE FROM key_values WHERE `key` = $key", map[string]any{
			"key": "test.exec.insert",
		})
		require.NoError(t, err)
	...
	t.Run("exec with no parameters", func(t *testing.T) {
		result, err := testDB.Exec("INSERT INTO key_values (`key`, value) VALUES ('test.no.params', 'no params')", map[string]any{})
		require.NoError(t, err)
		...
		_, err = testDB.Exec("DELETE FROM key_values WHERE `key` = 'test.no.params'", map[string]any{})
		require.NoError(t, err)
	})

	t.Run("exec with missing parameter", func(t *testing.T) {
		_, err := testDB.Exec("INSERT INTO key_values (`key`, value) VALUES ($key, $missing)", map[string]any{
			"key": "test.missing",
		})
		require.Error(t, err)
		require.Contains(t, err.Error(), "missing argument for named parameter: missing")
	...
	t.Run("exec with escaped dollar sign", func(t *testing.T) {
		result, err := testDB.Exec("INSERT INTO key_values (`key`, value) VALUES ('test.dollar', '$$escaped')", map[string]any{})
		require.NoError(t, err)
		...
		_, err = testDB.Exec("DELETE FROM key_values WHERE `key` = 'test.dollar'", map[string]any{})
		require.NoError(t, err)
	})
}
